...
class evaluation(object):
    def __init__(self):
        self.POS = []
        for i in range(SIZE):
            row = [(7 - max(abs(i - 7), abs(j - 7))) for j in range(SIZE)]
    ...
    def __evaluate(self, board, turn):
        ...
        if turn == WHITE:
            ...
            if count[1][THREE] > 1:
                ...
                bvalue += 100
            if count[2][STHREE]:
                wvalue += count[2][STHREE] * 10
            if count[1][STHREE]:
                bvalue += count[1][STHREE] * 10
            if count[2][TWO]:
                wvalue += count[2][TWO] * 4
            if count[1][TWO]:
                bvalue += count[1][TWO] * 4
            if count[2][STWO]:
                wvalue += count[2][STWO]
            if count[1][STWO]:
                bvalue += count[1][STWO]
    ...
    def __analysis_horizon(self, board, i, j):
        ...
        for x in range(SIZE):
            line[x] = board[i][x]
        self.analysis_line(line, result, SIZE, j)
        for x in range(SIZE):
            if result[x] != TODO:
    ...
    def __analysis_vertical(self, board, i, j):
        ...
        for x in range(SIZE):
            line[x] = board[x][j]
        self.analysis_line(line, result, SIZE, i)
        for x in range(SIZE):
            if result[x] != TODO:
    ...
    def __analysis_right(self, board, i, j):
        ...
        TODO = self.TODO
        if SIZE - 1 - i < j:
            x, y, realnum = j - SIZE + 1 + i, SIZE - 1, SIZE - 1 - i
        else:
            x, y, realnum = 0, i + j, j
        k = 0
        while k < SIZE:
            if x + k > SIZE - 1 or y - k < 0:
                break
            line[k] = board[y - k][x + k]
            k += 1
        self.analysis_line(line, result, k, j - x)
...
class searcher(object):
    ...
    def __init__(self):
        ...
        self.board = [[EMPTY for n in range(SIZE)] for i in range(SIZE)]
        self.gameover = 0
        self.overvalue = 0
        self.maxdepth = 3
    ...
    def get_action(self, chessboard=None, turn=WHITE, depth=2):
        self.maxdepth = depth
        self.bestmove = None
